                        ...
                        @if ($totalSegments)
                            @for ($i = 0; $i < $totalSegments; $i++)
                                <tr class="segment-row">
                                    <td>{!! Form::number('data[segment_style[number][]]', $i + 1, ['class' => 'form-control bg-dark text-light', 'readonly' => 'true']) !!}</td>
                                    <td>{!! Form::text('data[segment_style[text][]]', $segments[$i]['text'] ?? '', ['class' => 'form-control bg-dark text-light']) !!}</td>
                                    <td>
                                        <div class="input-group cp">
                                            {!! Form::text('data[segment_style[color][]]', $segments[$i]['fillStyle'] ?? null, ['class' => 'form-control']) !!}
